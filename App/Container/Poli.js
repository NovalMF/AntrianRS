    ...
    return (
        <View style={{backgroundColor: 'white', height: '100%', width: '100%'}}>
          ...
          <TextInput style={styles.inputs}
                    ...
                    />
                    <AntDesign name='search1' size={20} style={{ marginRight: 15 }} color={'#848484'}/>
          </View>
          <ScrollView style={{marginTop: 40}} >
            {
                this.state.poli_list.map((data, index)=>(
                  <TouchableOpacity key={index} onPress={() => this.props.navigation.navigate('Jadwaldokter', {poli_id: data.poli_id })}
                    style={this.state.poli_list.length - 1 === index ? {  backgroundColor: 'white', width: 330, height: 80, marginLeft: 16, marginTop: 10, borderRadius: 10, elevation: 5, marginBottom: 20 } : {  backgroundColor: 'white', width: 330, height: 80, marginLeft: 16, marginTop: 10, borderRadius: 10, elevation: 5 }}>
                      <Image source={{uri:data.icon_image}} style={{width: 60, height: 60, marginLeft:5, marginTop: 8}}></Image>
                      <Text style={{paddingLeft: 80, marginTop: -60, fontSize:18}}>{data.poli_nama}</Text>
